...
int main() {
  ...
  if (targetWidth <= 0 || targetHeight <= 0) { // Check that target width and height are positive integers
    cout << "Invalid target dimensions" << endl;
    return 1;
  }
  if (targetWidth < 1 || targetWidth > 1920) {
    cout << "Invalid target dimensions" << endl;
    return 1; // exit with error
  }
  if (targetHeight < 1 || targetHeight > 1080) {
    cout << "Invalid target dimensions" << endl;
    return 1; // exit with error
  }
  if (targetWidth > (int)width) {
    cout << "Invalid target dimensions" << endl;
    return 1; //exit with error
  }
  if (targetHeight > (int)height) {
    cout << "Invalid target dimensions" << endl;
    return 1; //exit with error
  }
