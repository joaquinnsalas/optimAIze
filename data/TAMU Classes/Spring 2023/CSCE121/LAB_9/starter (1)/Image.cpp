...
//Copy constructor
Image::Image(const Image& original) : image(nullptr), width(0), height(0) {
    // Step 1: allocate memory
    if (original.image != nullptr) {
        allocateImage(original.width, original.height);

        // Step 2: copy values

        for (unsigned int col = 0; col < width; col++) {
            for (unsigned int row = 0; row < height; row++) {
                image[col][row] = original.image[col][row];
            }
        }
    }
}
    ...
    // this->image = new allocateImage(width, height);
...
Image& Image::operator=(const Image& original) {
    ...
    // last step : return self
    return *this;
}

Image::~Image() { // Destructor (ensures no memory leak)
    clear();
    // Usually : Step 1 : deallocate memory
    //           Step 2 : 
}
